...
- **scan_dump.py** — один проход по дампу тем же блочным чтением, что и split_dump; собирает списки таблиц, индексов, функций и таблиц с данными (с количеством строк). Результат в JSON и краткая сводка в stdout (предпросмотр содержимого дампа без разбиения).
...
### 3. Сканирование дампа (предпросмотр)

Чтобы получить список таблиц, индексов, функций и таблиц с данными (и число строк по каждой) без разбиения дампа на чанки:

```bash
python3 scan_dump.py [-o scan_result.json] [--input-dir /bckp] [--dump-filename dump.sql]
```

- По умолчанию читается дамп из `INPUT_DIR/DUMP_FILENAME` (как в constants.py), результат пишется в `OUTPUT_DIR/scan_result.json`.
- Используется то же блочное чтение, что и в split_dump (`READ_BLOCK_BYTES`), так что объём памяти остаётся небольшим.
- Логи: `OUTPUT_DIR/scan_dump.log` и вывод в stdout (прогресс каждые 512 МБ и итоговая сводка).

### 4. Проверка и починка одной таблицы
...
- **Длинные строки (spill):** если неоконченная строка в памяти (`carry_over`) достигает 128 МБ (`MAX_CARRY_OVER_MEMORY`), она сбрасывается на диск и при следующем блоке стримится в выходной файл по 1 МБ, без загрузки в память. Иначе таблицы с очень длинными полями (HTML и т.п.) вызывали бы рост памяти до OOM на одном ряде (~40 ГБ в дампе).
